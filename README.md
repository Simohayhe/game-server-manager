...
- 別PC/スマホから: `config.yaml` を `api.host: 0.0.0.0`(+`api.password`)にしてサービス再起動、
  Windowsファイアウォールで受信TCPを許可 → スマホのブラウザで `http://<ホストIP>:8770/`。
  パスワードは右上 🔑 から入力(保存される)。
- **ポート番号なしで開きたい場合**: `api.web_port: 80` を足すと 80番でも同じ画面を配信するので
  `http://<ホストIP>/` で開ける(8770も従来通り使える)。80番が他アプリに使われていたり
  権限が無い場合は、その待受だけ諦めて本体はそのまま動く。
